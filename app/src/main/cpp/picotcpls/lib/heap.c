...
static void* map_in_pages(int page_count) {
    ...
    void* addr = mmap(NULL, page_count*pageSize, PROT_READ|PROT_WRITE, MAP_ANON|MAP_PRIVATE, -1, 0);

    if (addr == MAP_FAILED)
        return NULL;
    else {
        // Clear the memory
        memset(addr, 0, page_count*pageSize);
        //bzero(addr,page_count*pageSize);
...
static void map_out_pages(void* addr, int page_count) {
    ...
    int result = munmap(addr, page_count*pageSize);
...
void heap_create(heap* h, int initial_size, int (*comp_func)(void*,void*)) {
    ...
    if (pageSize == 0) {
        ...
        ENTRIES_PER_PAGE = pageSize / sizeof(heap_entry);
    }
    ...
    h->map_pages = sizeof(void*) * h->minimum_pages / pageSize + 1;
    ...
    for (int i=0;i<h->minimum_pages;i++) {
        *(h->mapping_table+i) = addr+(i*pageSize);
    }
...
void heap_insert(heap *h, void* key, void* value) {
    ...
    if (h->active_entries + 1 > max_entries) {
        ...
        int mapable_pages = map_pages * pageSize / sizeof(void*);
        ...
        if (h->allocated_pages + 1 > mapable_pages) {
            ...
            memcpy(new_table, old_table, map_pages * pageSize);
